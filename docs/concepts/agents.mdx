...
General AI Agents are the core building blocks of the Panda AGI SDK.
...
async def persistent_conversation():
    ...
    # First request
    async for event in agent.run("Tell me about Alice in Wonderland"):
        if event.type == "completed_task":
            break
    
    # Follow-up request with context
    async for event in agent.run("What's the main character's personality like?"):

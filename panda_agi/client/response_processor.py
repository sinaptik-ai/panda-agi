...
class ProcessorConfig:
    ...
    def __post_init__(self):
        ...
        if (
            self.xml_tool_calling is False
            and self.native_tool_calling is False
            and self.execute_tools
        ):
            raise ValueError(
                "At least one tool calling format (XML or native) must be enabled if execute_tools is True"
            )

        if self.xml_adding_strategy not in [
            "user_message",
            "assistant_message",
            "inline_edit",
        ]:
            raise ValueError(
                "xml_adding_strategy must be 'user_message', 'assistant_message', or 'inline_edit'"
            )

        if self.max_xml_tool_calls < 0:
            raise ValueError(
                "max_xml_tool_calls must be a non-negative integer (0 = no limit)"
            )
...
class ResponseProcessor:
    ...
    async def process_streaming_response(
        ...
        yielded_tool_indices = (
            set()
        )  # Stores indices of tools whose *status* has been yielded
        tool_index = 0
        xml_tool_call_count = 0
        finish_reason = None
        last_assistant_message_object = (
            None  # Store the final saved assistant message object
        )
        tool_result_message_objects = {}  # tool_index -> full saved message object
        has_printed_thinking_prefix = (
            False  # Flag for printing thinking prefix only once
        )

        logger.info(
            f"Streaming Config: XML={config.xml_tool_calling}, Native={config.native_tool_calling}, "
        ...
        try:
            ...
            async for chunk in llm_response:
                ...
                if hasattr(chunk, "choices") and chunk.choices:
                    ...
                    if delta and hasattr(delta, "content") and delta.content:
                        ...
                        if not (
                            ...
                            logger.info(
                                "XML tool call limit reached - not yielding more content chunks"
                            )
                        ...
                        if config.xml_tool_calling and not (
                            ...
                            for xml_chunk in xml_chunks:
                                current_xml_content = current_xml_content.replace(
                                    xml_chunk, "", 1
                                )
                                ...
                                if result:
                                    ...
                                    if (
                                        ...
                                        and config.execute_on_stream
                                    ):
                                        # Save and Yield tool_started status
                                        started_msg_obj = (
                                            await self._yield_and_save_tool_started(
                                                context, thread_id, thread_run_id
                                            )
                                        )
                                        if started_msg_obj:
                                            yield started_msg_obj
                                        yielded_tool_indices.add(
                                            tool_index
                                    ...
                                    if (
                                        config.max_xml_tool_calls > 0
                                        and xml_tool_call_count
                                        >= config.max_xml_tool_calls
                                    ):
                                        logger.debug(
                                            f"Reached XML tool call limit ({config.max_xml_tool_calls})"
                                        )
                    ...
                    if (
                        ...
                        for tool_call_chunk in delta.tool_calls:
                            ...
                            if hasattr(tool_call_chunk, "model_dump"):
                                ...
                                if hasattr(tool_call_chunk, "function"):
                                    ...
                                    if hasattr(tool_call_chunk.function, "name"):
                                        tool_call_data_chunk["function"]["name"] = (
                            ...
                            if (
                                tool_calls_buffer.get(idx)
                                and tool_calls_buffer[idx]["id"]
                                and tool_calls_buffer[idx]["function"]["name"]
                                and tool_calls_buffer[idx]["function"]["arguments"]
                            ):
                                try:
                                    json.loads(
                            ...
                            if (
                                ...
                                execution_task = asyncio.create_task(
                                    self._execute_tool(tool_call_data)
                                )
                                pending_tool_executions.append(
                                    {
                                        "task": execution_task,
                ...
                if finish_reason == "xml_tool_limit_reached":
                    logger.info(
                        "Stopping stream processing after loop due to XML tool call limit"
                    )
                    break
            ...
            if pending_tool_executions:
                ...
                # ... (asyncio.wait logic) ...
                pending_tasks = [
                    execution["task"] for execution in pending_tool_executions
                ]
                ...
                for execution in pending_tool_executions:
                    ...
                    if tool_idx in yielded_tool_indices:
                        logger.debug(
                            f"Status for tool index {tool_idx} already yielded."
                        )
                    ...
                    try:
                        if execution["task"].done():
                            ...
                            completed_msg_obj = (
                                await self._yield_and_save_tool_completed(
            ...
            if accumulated_content:
                ...
                if (
                    config.max_xml_tool_calls > 0
                    and xml_tool_call_count >= config.max_xml_tool_calls
                    and xml_chunks_buffer
                ):
                    last_xml_chunk = xml_chunks_buffer[-1]
                    last_chunk_end_pos = accumulated_content.find(last_xml_chunk) + len(
                        last_xml_chunk
                    )
                ...
                if config.native_tool_calling:
                    for idx, tc_buf in tool_calls_buffer.items():
                        if (
                            tc_buf["id"]
                            and tc_buf["function"]["name"]
                            and tc_buf["function"]["arguments"]
                        ):
                            try:
            ...
            if config.execute_tools:
                ...
                if config.xml_tool_calling:
                    ...
                    remaining_limit = (
                        config.max_xml_tool_calls - xml_tool_call_count
                        if config.max_xml_tool_calls > 0
                        else len(xml_chunks_buffer)
                    )
                    xml_chunks_to_process = xml_chunks_buffer[
                        :remaining_limit
                    ...
                    for chunk in xml_chunks_to_process:
                        ...
                        if parsed_result:
                            ...
                            if not any(
                                ...
                                for exec in pending_tool_executions
                            ):
                                final_tool_calls_to_process.append(tool_call)
                                parsed_xml_data.append(
                                    {
                                        "tool_call": tool_call,
                ...
                if config.execute_on_stream and tool_results_buffer:
                    logger.info(
                        f"Processing {len(tool_results_buffer)} buffered tool results"
                    )
                ...
                # Or execute now if not streamed
                    ...
                    for tc, res in results_list:
                        ...
                        if current_tool_idx in all_tool_data_map:
                            ...
                            logger.warning(
                                f"Could not map result for tool index {current_tool_idx}"
                            )
                        current_tool_idx += 1
                ...
                if tool_results_map:
                    ...
                    for tool_idx in sorted(tool_results_map.keys()):
                        ...
                        context.result = result
                        if (
                            not context.assistant_message_id
                            and last_assistant_message_object
                        ):
                            context.assistant_message_id = (
                                last_assistant_message_object["message_id"]
                            )
                        ...
                        if (
                            not config.execute_on_stream
                            and tool_idx not in yielded_tool_indices
                        ):
                        ...
                        if saved_tool_result_object:
                            tool_result_message_objects[tool_idx] = (
                                saved_tool_result_object
                            )
            ...
            if (
                last_assistant_message_object
            ):  # Only calculate if assistant message was saved
                try:
                    ...
                    if final_cost is not None and final_cost > 0:
                        ...
                        logger.info(
                            "Stream cost calculation resulted in zero or None, not storing cost message."
                        )
                except Exception as e:
        ...
        try:
                ...
                if response_message:
                    if (
                        ...
                        if config.xml_tool_calling:
                            ...
                            if (
                                config.max_xml_tool_calls > 0
                                and len(parsed_xml_data) > config.max_xml_tool_calls
                            ):
                                ...
                                if parsed_xml_data:
                                    ...
                                    if xml_chunks:
                                        ...
                                        if last_chunk_pos >= 0:
                                            content = content[
                                                : last_chunk_pos + len(last_chunk)
                                            ]
                                parsed_xml_data = parsed_xml_data[
                                    : config.max_xml_tool_calls
                                ]
                                finish_reason = "xml_tool_limit_reached"
            ...
            if (
                ...
                try:
                    ...
                    final_cost = None
                    if (
                        hasattr(llm_response, "_hidden_params")
                        and "response_cost" in llm_response._hidden_params
                        and llm_response._hidden_params["response_cost"] is not None
                        and llm_response._hidden_params["response_cost"] != 0.0
                    ):
                        final_cost = llm_response._hidden_params["response_cost"]
                        logger.info(
                            f"Using response_cost from _hidden_params: {final_cost}"
                        )

                    if (
                        final_cost is None
                    ):  # Fall back to calculating cost if direct cost not available or zero
                    ...
                    if final_cost is not None and final_cost > 0:
                        logger.info(
                            f"Calculated final cost for non-stream: {final_cost}"
                        )
                        await self.add_message(
                            thread_id=thread_id,
                            type="cost",
                        ...
                        logger.info(
                            "Non-stream cost calculation resulted in zero or None, not storing cost message."
                        )

                except Exception as e:
                    logger.error(
                        f"Error calculating final cost for non-stream: {str(e)}"
                    )
            ...
            if config.execute_tools and tool_calls_to_execute:
                logger.info(
                    f"Executing {len(tool_calls_to_execute)} tools with strategy: {config.tool_execution_strategy}"
                )
                tool_results = await self._execute_tools(
                    tool_calls_to_execute, config.tool_execution_strategy
                )
                ...
                for i, (returned_tool_call, result) in enumerate(tool_results):
                    ...
                    current_assistant_id = (
                        assistant_message_object["message_id"]
                        if assistant_message_object
                        else None
                    )
                    ...
                    # Yield the saved tool result object
                    if saved_tool_result_object:
                        tool_result_message_objects[tool_index] = (
                            saved_tool_result_object
                        )
                        yield saved_tool_result_object
                    else:
                        logger.error(
                            f"Failed to save tool result for index {tool_index}"
                        )
            ...
            logger.error(
                f"Error processing non-streaming response: {str(e)}", exc_info=True
            )
    ...
    def _extract_tag_content(
        self, xml_chunk: str, tag_name: str
    ...
    def _extract_attribute(self, opening_tag: str, attr_name: str) -> Optional[str]:
        ...
        try:
            ...
            patterns = [
                rf'{attr_name}="([^"]*)"',  # Double quotes
                rf"{attr_name}='([^']*)'",  # Single quotes
                rf"{attr_name}=([^\s/>;]+)",  # No quotes - fixed escape sequence
            ]
        ...
        try:
            while pos < len(content):
                ...
                for tag_name in self.tool_registry.xml_tools.keys():
                    ...
                    if tag_pos != -1 and (
                        next_tag_start == -1 or tag_pos < next_tag_start
                    ):
    ...
    def _parse_xml_tool_call(
        ...
        try:
                ...
                try:
                    if mapping.node_type == "attribute":
                        ...
                        if value is not None:
                            ...
                            parsing_details["attributes"][mapping.param_name] = (
                                value  # Store raw attribute
                            )
                            # logger.info(f"Found attribute {mapping.param_name}: {value}")

                    elif mapping.node_type == "element":
                        # Extract element content
                        content, remaining_chunk = self._extract_tag_content(
                            remaining_chunk, mapping.path
                        )
                        if content is not None:
                            params[mapping.param_name] = content.strip()
                            parsing_details["elements"][mapping.param_name] = (
                                content.strip()
                            )  # Store raw element content
                            # logger.info(f"Found element {mapping.param_name}: {content.strip()}")

                    elif mapping.node_type == "text":
                        # Extract text content
                        content, _ = self._extract_tag_content(
                            remaining_chunk, xml_tag_name
                        )
                        if content is not None:
                            params[mapping.param_name] = content.strip()
                            parsing_details["text_content"] = (
                                content.strip()
                        ...
                        if content is not None:
                            params[mapping.param_name] = content.strip()
                            parsing_details["root_content"] = (
                                content.strip()
                            )  # Store raw root content
            ...
            missing = [
                mapping.param_name
                for mapping in schema.mappings
                if mapping.required and mapping.param_name not in params
            ]
            if missing:
        ...
        try:
            ...
            for xml_chunk in xml_chunks:
                ...
                if result:
                    tool_call, parsing_details = result
                    parsed_data.append(
                        {"tool_call": tool_call, "parsing_details": parsing_details}
                    )
    ...
    async def _execute_tool(self, tool_call: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            if not tool_fn:
                ...
                return ToolResult(
                    success=False, output=f"Tool function '{function_name}' not found"
                )
            ...
            logger.error(
                f"Error executing tool {tool_call['function_name']}: {str(e)}",
                exc_info=True,
            )
    ...
    async def _execute_tools(
        ...
        """
        logger.info(
            f"Executing {len(tool_calls)} tools with strategy: {execution_strategy}"
        )
    ...
    async def _execute_tools_sequentially(
        ...
        try:
            ...
            for index, tool_call in enumerate(tool_calls):
                ...
                logger.debug(
                    f"Executing tool {index + 1}/{len(tool_calls)}: {tool_name}"
                )
                ...
                try:
                    ...
                    logger.debug(
                        f"Completed tool {tool_name} with success={result.success}"
                    )
                except Exception as e:
                    logger.error(f"Error executing tool {tool_name}: {str(e)}")
                    error_result = ToolResult(
                        success=False, output=f"Error executing tool: {str(e)}"
                    )
            ...
            completed_tool_names = (
                [r[0].get("function_name", "unknown") for r in results]
            ...
            remaining_tools = [
                t
                for t in tool_calls
                if t.get("function_name", "unknown") not in completed_tool_names
            ]

            # Add error results for remaining tools
            error_results = [
                (tool, ToolResult(success=False, output=f"Execution error: {str(e)}"))
        ...
        try:
            ...
            for i, (tool_call, result) in enumerate(zip(tool_calls, results)):
                if isinstance(result, Exception):
                    ...
                    # Create error result
                    error_result = ToolResult(
                        success=False, output=f"Error executing tool: {str(result)}"
                    )
            ...
            return [
                (
                    tool_call,
                    ToolResult(success=False, output=f"Execution error: {str(e)}"),
                )
    ...
    async def _add_tool_result(
        ...
        try:
            ...
            if assistant_message_id:
                ...
                logger.info(
                    f"Linking tool result to assistant message: {assistant_message_id}"
                )
            ...
            context = self._create_tool_context(
                tool_call, 0, assistant_message_id, parsing_details
            )
            context.result = result
            ...
            try:
                ...
                logger.error(
                    f"Failed even with fallback message: {str(e2)}", exc_info=True
                )
                return None  # Return None on error

    def _format_xml_tool_result(
    ...
    async def _yield_and_save_tool_started(
        ...
        content = {
            ...
            "tool_call_id": context.tool_call.get(
                "id"
            ),  # Include tool_call ID if native
        }
        metadata = {"thread_run_id": thread_run_id}
    ...
    async def _yield_and_save_tool_error(
        ...
        error_msg = (
            str(context.error)
            if context.error
            else "Unknown error during tool execution"
        )
        tool_name = context.xml_tag_name or context.function_name
        content = {
            "role": "assistant",
